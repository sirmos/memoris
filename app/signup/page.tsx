    ...
    <>
      <style>{`
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .eye-btn { position: absolute; right: 14px; top: 50%; transform: translateY(-50%); background: none; border: none; color: #6b6058; cursor: pointer; display: flex; padding: 0; }
        ...
        .terms { font-size: 12px; color: #4a4540; text-align: center; margin-top: 16px; line-height: 1.6; }
        .terms a { color: #C9A86A; text-decoration: none; }
      ...
      <div className="page">
        ...
        <div className="box">
          ...
          <p>Start organizing your digital legacy today</p>
          {error && <div className="error">{error}</div>}
          <div className="field"><label>Full name</label><input type="text" placeholder="John Enoch" value={name} onChange={e => setName(e.target.value)} /></div>
          ...
          <div className="field">
            ...
            <div className="input-wrap">
              <input type={showPassword ? 'text' : 'password'} placeholder="Min. 8 characters" value={password} onChange={e => setPassword(e.target.value)} style={{paddingRight:'44px'}} />
              <button className="eye-btn" onClick={() => setShowPassword(!showPassword)}>{showPassword ? <EyeOff size={17} /> : <Eye size={17} />}</button>
            </div>
          </div>
          <div className="field">
            <label>Confirm password</label>
            <div className="input-wrap">
              <input type={showConfirm ? 'text' : 'password'} placeholder="Re-enter your password" value={confirm} onChange={e => setConfirm(e.target.value)} style={{paddingRight:'44px'}} />
              <button className="eye-btn" onClick={() => setShowConfirm(!showConfirm)}>{showConfirm ? <EyeOff size={17} /> : <Eye size={17} />}</button>
            </div>
          </div>
          ...
          <p className="terms">By creating an account you agree to our <a href="#">Terms of Service</a> and <a href="#">Privacy Policy</a></p>
          <div className="divider"><span>or</span></div>
          <div className="switch">Already have an account? <a href="/signin">Sign in</a></div>
        </div>
      </div>
    </>
  )
